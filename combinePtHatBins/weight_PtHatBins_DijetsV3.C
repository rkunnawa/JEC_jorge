...
void weight_PtHatBins_DijetsV3(const char *infile="input.root", const char *outfile = "output.root",float maxpthat=30., float xSection=1.,bool updateCorrections=true)
{
  ...
  //  fin->cd(inputDirName[idir].Data());
   ...
   // float weight = xSection/fentries;
	...
	if(updateCorrections){
	  ...
	  corrpt[iref] = rawpt[iref]*(_JEC_HI44X->getCorrection()); 
 if(corrpt[iref]<0  && rawpt[iref]>15)cout<<"iref: "<<iref<<" correction is: "<<(_JEC_HI44X->getCorrection())<<" for rawpt: "<<rawpt[iref]<<" corrpt: "<<corrpt[iref]<<" and rawpT*correction: "<<(rawpt[iref]*_JEC_HI44X->getCorrection())<<" and correction*rawpt: "<<(_JEC_HI44X->getCorrection()*rawpt[iref])<<" and eta: "<<jteta[iref]<<endl;
	}
      }//ref for loop
      tr_out[idir]->Fill();
    }//nentresi loop
