 ...
 //   Int_t i = 2 ;
  ...
  //  for(int i =1; i<nCanvDiv-1; i++)
   ...
   i2 = i+6; 
    ...
    TTree* tt2      = (TTree*)FileA->Get(Form("%sJetAnalyzer/t", AlgoNames[i]));
     ...
     iHistoCorrPt[i] = new TH2D(Form("%s_CorrPt",AlgoNames[i]),Form("%s_CorrPt",AlgoNames[i]),nJetPtBin,jetPtBin,500,0,5);
      iHistoCorrPt[i+6] = new TH2D(Form("%s_CorrPt",AlgoNames[i+6]),Form("%s_CorrPt",AlgoNames[i+6]),nJetPtBin,jetPtBin,500,0,5);
        iHistoCorrPt[i]->Sumw2();
      iHistoCorrPt[i+6]->Sumw2();
  //      t->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i]),"(corrpt>0 && refpt>30 && fabs(jteta)<3.0 && rawpt>15.)","goff");
        t->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i]),"weight*(corrpt>0 && refpt>30 && fabs(jteta)<2.0 && rawpt>15.)","goff");
      //tt2->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i+6]),"weight*(corrpt>0 && refpt>30 && fabs(jteta)<2.0 && rawpt>15.)","same");
 //    t->Draw(Form("corrpt/refpt:refpt>>%s_CorrPt",AlgoNames[i]),Form("corrpt>0 && fabs(jteta)<3.0"),"goff");
      iHistoCorrPt[i]->Draw("colz");
        iHistoCorrPt[i+6]->Draw("same colz");
        leg[i]->AddEntry(iHistoCorrPt[i],Form("%s |#eta|<3.0",AlgoNames[i]),"");
      leg[i]->AddEntry(iHistoCorrPt[i+6],Form("%s |#eta|<3.0",AlgoNames[i+6]),"");
      ...
      format1Dhisto(*iHistoCorrPt[i],250,-1,2,20,2,1,"Jet p_{T}^{ref} (GeV/c)","Corrected p_{T}/ ref p_{T}");//Red      
        //format1Dhisto(*iHistoCorrPt[i+6],250,-1,2,20,2,1,"Jet p_{T}^{ref} (GeV/c)","Corrected p_{T}/ ref p_{T}");//Red
      leg[i]->Draw();
  //    for(int iX=iHistoCorrPt[i]->GetXaxis()->GetFirst(); iX<iHistoCorrPt[i]->GetXaxis()->GetLast(); iX++ )
          for(int iX=0; iX<nJetPtBin; iX++ )
	{
	  ...
	  h1[i][iX] =(TH1D*)iHistoCorrPt[i]->ProjectionY(Form("h1_%d_%d_py",i,iX),iX,iX,"e");
        ...
        h1[i+6][iX] =(TH1D*)iHistoCorrPt[i+6]->ProjectionY(Form("h1_%d_%d_py",i+6,iX),iX,iX,"e");
...
//    h1[i][iX] =(TH1D*)iHistoCorrPt[i]->ProjectionY(Form("h1_%s_Pt%f_%f_py",AlgoNames[i],jetPtBin[iX],jetPtBin[iX+1]),iHistoCorrPt[i]->GetXaxis()->FindBin(jetPtBin[iX]),iHistoCorrPt[i]->GetXaxis()->FindBin(jetPtBin[iX+1])-1,"e");
	  ...
	  h1[i][iX]->Fit(Form("F6_c%d_d%d",i,iX),"R0WL", "", 0.7, 1.5);
        Gauss6[i+6][iX] = new TF1(Form("F6_c%d_d%d",i+6,iX),"gaus",0,3);
        Gauss6[i+6][iX]->SetParLimits(1,0.7,1.5);
        h1[i+6][iX]->Fit(Form("F6_c%d_d%d",i+6,iX),"R0WL", "NO", 0.7, 1.5);
     //   h1[i][iX]->Fit(Gauss6[i][iX],"0Q");
	  ...
	  xPointErr[i][iX] = 0;
        xPoint[i+6][iX] =iHistoCorrPt[i+6]->GetXaxis()->GetBinCenter(iX);
        xPointErr[i+6][iX] = 0;
...
//     if(i2>=12)c3C->cd(i2-12);
    ...
    if(SavePlot){
        c2->Print(Form("%s/Pbp502CorrectedOverRefJetMean.pdf", plotsdir.Data()));
        c3->Print(Form("%s/Pbp502CorrectedOverRefJetSigma.pdf",plotsdir.Data())) ;
    }
    c2->Update();
    c3->Update();
}
