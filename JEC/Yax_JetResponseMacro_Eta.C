...
//    TFile *FileA = TFile::Open(Form("/Users/ymao/group/CMS/anaOutputs/pPb/JEC/corrected_dijet_pp_mergedpthatbins_2013MCV3.root"));
 //   TFile *FileA = TFile::Open(Form("/Users/ymao/group/CMS/anaOutputs/pPb/JEC/dijet_Corrpp_mergedpthatbins_2013MC.root"));
    TFile *FileA = TFile::Open(Form("/Users/ymao/group/CMS/anaOutputs/JEC/jet_Corrpp276HITrack_mergedpthatbins_2013MC.root"));
...
//  makeMultiPanelCanvas(c1,cCanDiv,2,0.0,0.0,0.2,0.15,0.07);
  ...
  //  for(int i =1; i<nCanvDiv-1; i++)
    ...
    TTree* t      = (TTree*)FileA->Get(Form("%sJetAnalyzer/t", AlgoNames[i]));
      ...
      leg[i]->SetBorderSize(0);
        ...
        iHistoCorrEta[i+6] = new TH2D(Form("%s_CorrEta",AlgoNames[i+6]),Form("%s_CorrEta",AlgoNames[i+6]),nXbins,etabins,500,0,5);
 ...
 //       t->Draw(Form("corrpt/refpt:jteta>>%s_CorrEta",AlgoNames[i]),Form("(rawpt>15 && refpt>30 && fabs(jteta)<3.0)"),"goff");
       t->Draw(Form("corrpt/refpt:refeta>>%s_CorrEta",AlgoNames[i]),Form("weight*(corrpt>0 && refpt>50 && fabs(jteta)<3.0 && rawpt>15.)"),"goff");
        tt2->Draw(Form("corrpt/refpt:refeta>>%s_CorrEta",AlgoNames[i+6]),Form("weight*(corrpt>0 && refpt>50 && fabs(jteta)<3.0 && rawpt>15.)"),"same");
    ...
    if(SavePlot){
        c1->Print(Form("%s/pp276HITrackCorrectedEtaOverRefJet.pdf", plotsdir.Data()));
    }
...
//     if(i2>=12)c3C->cd(i2-12);
    ...
    if(SavePlot){
        c2->Print(Form("%s/pp276HITrackCorrectedEtaOverRefJetMean.pdf", plotsdir.Data()));
        c3->Print(Form("%s/pp276HITrackCorrectedEtaOverRefJetSigma.pdf",plotsdir.Data())) ; 
    }
    c2->Update();
    c3->Update();
}
