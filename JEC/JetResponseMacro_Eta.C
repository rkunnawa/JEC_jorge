...
void JetResponseMacro_Eta(){
        ...
        //  for(int i =1; i<nCanvDiv-1; i++)
    {
        ...
        iHistoCorrEta[i] = new TH2D(Form("%s_CorrEta",AlgoNames[i]),Form("%s_CorrEta",AlgoNames[i]),nXbins,-3.2,3.2,500,0,5);
        ...
        t->Draw(Form("corrpt/refpt:jteta>>%s_CorrEta",AlgoNames[i]),Form("weight*(rawpt>15 && refpt>30 && fabs(jteta)<3.0)"),"goff");
        //t->Draw(Form("corrpt/refpt:jteta>>%s_CorrEta",AlgoNames[i]),Form("weight*(rawpt>30 && fabs(jteta)<3.0 && rawpt>15"),"goff");
    ...
    if(SavePlot){
        c1->Print(Form("CorrectedEtaOverRefJet.pdf"));
    }
    ...
    if(SavePlot){
        c2->Print(Form("CorrectedEtaOverRefJetMean.pdf"));
        c3->Print(Form("CorrectedEtaOverRefJetSigma.pdf")) ;
    }
    c2->Update();
    c3->Update();
}
